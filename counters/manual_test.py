...
from counters import CountupTimer, CountupTimerWithExpiry, CountdownTimerWithExpiry
...
def test_countupexpiry_print(iterations: int=10, duration: int=0.75):
    ...
    for _ in range(8):
        print(f"{round(datetime.now().second,0)}\t\t{round(countup_timer.elapsed, 3)}\t\t{countup_timer.paused}", end="\r")
        time.sleep(1)
    countup_timer.pause()
    for _ in range(8):
        print(f"{datetime.now().second}\t\t{round(countup_timer.elapsed, 3)}\t\t{countup_timer.paused}", end="\r")
        time.sleep(1)
    countup_timer.resume()
    for _ in range(8):
        print(f"{datetime.now().second}\t\t{round(countup_timer.elapsed, 3)}\t\t{countup_timer.paused}", end="\r")
